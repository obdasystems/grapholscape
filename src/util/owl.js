export default class OwlTranslator {
  constructor() { }
  ...
  edgeToOwlString (edge) {
    ...
    switch (edge.data('type')) {
      case 'inclusion':
        if (source.data('identity') == 'concept' && target.data('identity') == 'concept') {
          ...
          if (target.data('type') == 'complement') {
            return disjointTypeProperties(this, edge)
          } else { return subTypePropertyOf(this, edge) }
      ...
      case 'equivalence':
        if (source.data('identity') == 'concept' && target.data('identity') == 'concept') {
          ...
          if (source.data('type') == 'role-inverse' || target.data('type') == 'role-inverse') { return inverseObjectProperties(this, edge) } else { return equivalentTypeProperties(this, edge) }
        } else if (source.data('identity') == 'attribute' && target.data('identity') == 'attribute') {
      ...
      case 'membership':
        if (target.data('identity') == 'concept') { return classAssertion(this, edge) } else { return propertyAssertion(this, edge) }
        break
    }
    ...
    function propertyDomain (self, edge) {
      ...
      if (node.size() > 1) { return subClassOf(self, edge) }

      if (node.data('type') == 'role') { return 'ObjectPropertyDomain(' + self.nodeToOwlString(node) + ' ' + self.nodeToOwlString(edge.target()) + ')' } else if (node.data('type') == 'attribute') { return 'DataPropertyDomain(' + self.nodeToOwlString(node) + ' ' + self.nodeToOwlString(edge.target()) + ')' }
    }
    ...
    function propertyRange (self, edge) {
      ...
      if (node.data('type') == 'role') { return 'ObjectPropertyRange(' + self.nodeToOwlString(node) + ' ' + self.nodeToOwlString(edge.target()) + ')' } else if (node.data('type') == 'attribute') { return 'DataPropertyRange(' + self.nodeToOwlString(node) + ' ' + self.nodeToOwlString(edge.target()) + ')' }
    }
  ...
  nodeToOwlString (node, from_node) {
    ...
    function someValuesFrom (self, first, other, restr_type) {
      ...
      if (restr_type == 'range-restriction') { owl_string += objectInverseOf(self, first) } else { owl_string += self.nodeToOwlString(first) }
    ...
    function minMaxExactCardinality (self, first, other, cardinality, restr_type) {
      ...
      if (cardinality[0] == '-') {
        if (restr_type == 'range-restriction') {
          if (!other) { return axiom_type + 'MaxCardinality(' + cardinality[1] + ' ' + objectInverseOf(self, first) + ')' } else { return axiom_type + 'MaxCardinality(' + cardinality[1] + ' ' + objectInverseOf(self, first) + ' ' + self.nodeToOwlString(other) + ')' }
        } else {
          if (!other) { return axiom_type + 'MaxCardinality(' + cardinality[1] + ' ' + self.nodeToOwlString(first) + ')' } else { return axiom_type + 'MaxCardinality(' + cardinality[1] + ' ' + self.nodeToOwlString(first) + ' ' + self.nodeToOwlString(other) + ')' }
        }
      }

      if (cardinality[1] == '-') {
        if (restr_type == 'range-restriction') {
          if (!other) { return axiom_type + 'MinCardinality(' + cardinality[0] + ' ' + objectInverseOf(self, first) + ')' } else { return axiom_type + 'MinCardinality(' + cardinality[0] + ' ' + objectInverseOf(self, first) + ' ' + self.nodeToOwlString(other) + ')' }
        } else {
          if (!other) { return axiom_type + 'MinCardinality(' + cardinality[0] + ' ' + self.nodeToOwlString(first) + ')' } else { return axiom_type + 'MinCardinality(' + cardinality[0] + ' ' + self.nodeToOwlString(first) + ' ' + self.nodeToOwlString(other) + ')' }
        }
      }
    ...
    function hasSelf (self, input, restr_type) {
      ...
      if (restr_type == 'range-restriction') { return 'ObjectHasSelf(' + objectInverseOf(self, input) + ')' }
