...
export function getNewEndpoint(endpoint: Position, node: GrapholNode, breakpoint: Position) {
  ...
  const breakpointRelativeToNode = new Breakpoint()
  breakpointRelativeToNode.x = breakpoint.x - node.x
  breakpointRelativeToNode.y = breakpoint.x - node.y
  ...
  if (endpointRelativeToNode.x == breakpoint.x) {
  ...
  else if (endpointRelativeToNode.y == breakpoint.y) {
    if (breakpoint.x > 0) {
