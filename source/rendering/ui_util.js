...
export function toggle (button) {
  if (button.classList.contains('bottom_button')) {
    ...
    for (i = 0; i < windows.length; i++) {
      if (button.parentNode.getElementsByClassName('bottom_window')[0] !== windows[i]) { windows[i].classList.add('hide') }
    }
    ...
    if (elm.id === 'slider_body') { button.parentNode.getElementsByTagName('hr')[0].classList.toggle('hide') }
  }
}
